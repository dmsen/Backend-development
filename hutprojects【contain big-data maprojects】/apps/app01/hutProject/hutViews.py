...
def test_user_get(request):
    from .hutModel import hutProjectsUsers,hutNotice
    jsonResponse = json.loads(request.body.decode('utf-8'))
    # print("测试用户get——————————————————")
    # print (jsonResponse)
    ...
    for i in range(len(pastJson)):
        # print( queryset_to_json(hutNotice.objects.filter(hutNoticeUser=pastJson[i].__dict__['id'])).status)
        # 此用户接受到的通知
        from django.db.models import Q
        # noticeLists = len(queryset_to_json(hutNotice.objects.filter( Q(hutNoticeUser=pastJson[i].__dict__['id'])  & Q(status=0) )))
        # print("测试")
        # print(noticeLists)
        lists.append({
            'id': pastJson[i].__dict__['id'],
            'num':len(queryset_to_json(hutNotice.objects.filter( Q(hutNoticeUser=pastJson[i].__dict__['id'])  & Q(status=0) ))),
...
def test_user_getNotice(request):
    ...
    from django.db.models import Q
    pastJson = queryset_to_json(hutNotice.objects.filter( Q(hutNoticeUser=jsonResponse['userId'])  & Q(status=0) ))
            ...
            for i in range(len(pastJson)):
                jsonLists.append(
                    {"content": pastJson[i]["msg"],
                     ...
                     "hutNoticeId":pastJson[i]["hutNoticeId"],
                     }
                )
        else:
...
def test_user_readNotice(request):
    from .hutModel import hutNotice
    jsonResponse = json.loads(request.body.decode('utf-8'))
    print (jsonResponse['noticeId'],type(jsonResponse['noticeId']))
    print("测试））））））））））））））））））））））））））））")
    try:
        hutNotice.objects.filter(hutNoticeId=jsonResponse['noticeId']).update(
            status = 1
        )
    except:
        return JsonResponse({"result": 1, "msg": "未知错误"})
...
def test_user_readEdNotice(request):
    from .hutModel import hutNotice
    jsonResponse = json.loads(request.body.decode('utf-8'))
    from django.db.models import Q
    pastJson = queryset_to_json(hutNotice.objects.filter( Q(hutNoticeUser=jsonResponse['userId'])  & Q(status=1) ))
    pastJsonLength = len(pastJson)
    jsonLists = []
    if(len(pastJson) == 0):
        return JsonResponse({"result": 1, "msg": "为空"})
